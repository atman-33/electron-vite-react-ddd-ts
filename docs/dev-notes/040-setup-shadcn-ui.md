...
`tsconfig.web.json`パスエイリアスをそのまま利用する。
...
```json
{
  ...
  "aliases": {
    "components": "@renderer/components/shadcn-ui",
...
shadcn CLIは、tsconfig.jsonのrefferencesに対応していないため、専用のコマンドを作成する。
...
> `add-component`はshell、`add-component-ps`はpowershell向けのコマンド

### Buttonをインストール

```bash
npm run add-component-ps button
```
...
## eslint, prettierの対象から除外

`.eslintignore`

```text
# shadcn/ui
src/renderer/src/components/shadcn/ui
```
